...
PLIST="com.memory-router.agent-log-sync.plist"
LABEL="${PLIST%.plist}"
INTERVAL_SECONDS="${MEMORY_ROUTER_AGENT_LOG_SYNC_INTERVAL_SECONDS:-3600}"
...
run_once() {
  cd "$PROJECT_ROOT"
  "$(resolve_bun_path)" run src/cli/sync-agent-logs.ts
}
