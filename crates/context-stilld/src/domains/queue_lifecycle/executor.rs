...
fn run_split_provider_executor_tick_for_path(
    ...
    while claimed < config.max_claims {
        ...
        if setup.job.queue_name == "landscapeCuration" {
            ...
            let embedding = finalize_embedding_config(&reader, &settings, &config);
            match super::curation_executor::run_for_path(
                sqlite_path,
                setup.job.clone(),
                setup.target.clone(),
                setup.api_key.as_deref().map(|s| s.as_str()),
                ...
                Ok(false) => retried += 1,
                Err(error) => {
                    super::curation_executor::fail_for_path(
                        sqlite_path,
                        setup.job.clone(),
                        error.to_string(),
                    )?;
...
fn run_executor_tick_with_connection(
    ...
    for pool in pools {
        ...
        let priority_queues =
            executor_priority_queues_for_pool(&settings, &pool.pool_id, &paused_queues)
                .into_iter()
                .filter(|q| q.queue_name != "landscapeCuration")
                .collect::<Vec<_>>();
        if priority_queues.is_empty() {
            continue;
        }
...
fn rust_provider_executor_supports_queue(queue_name: &str) -> bool {
    matches!(
        queue_name,
        "findingCandidate" | "episodeDistiller" | "landscapeCuration"
    )
}
...
pub(crate) fn rust_executor_supports_queue(queue_name: &str) -> bool {
    matches!(
        queue_name,
        "findingCandidate" | "episodeDistiller" | "landscapeCuration" | "finalizeDistille"
    )
}
...
fn queue_spec_for_pool(
    ...
    match queue_name {
        ...
        "landscapeCuration" => simple_route_spec(
            settings,
            queue_name,
            settings.pointer("/taskRouting/landscapeCuration"),
            pool_id,
        ),
