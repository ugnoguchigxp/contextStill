...
export const doctorReasonCatalog: Record<string, DoctorReasonCatalogEntry> = {
  ...
  CONTEXT_DECISION_TABLES_MISSING: {
    label: "context decision tables が不足",
    severity: "warning",
    area: "Runtime",
    description: "context_decision の永続化テーブルが DB に存在しません。",
    impact: "context_decision tool と Decision UI が判断履歴を保存できません。",
    action: "database migration を実行し、context_decision tables を作成してください。",
  },
  CONTEXT_DECISION_ESCALATE_RATE_HIGH: {
    label: "Context decision escalate rate が高い",
    severity: "warning",
    area: "Runtime",
    description: "context_decision の escalate 比率が目標値を超えています。",
    impact: "自律判断がユーザー確認に戻りやすくなっています。",
    action: "degraded decisions と不足 Knowledge evidence を確認してください。",
  },
  CONTEXT_DECISION_FEEDBACK_EFFECTS_QUEUED: {
    label: "Context decision feedback effects が滞留",
    severity: "warning",
    area: "Knowledge",
    description: "曖昧な context_decision feedback effect が review 待ちです。",
    impact: "feedback が Knowledge scoring サイクルへ戻るまで遅延します。",
    action: "Decision UI または review queue で queued effects を確認してください。",
  },
  CONTEXT_DECISION_REQUIRED_ZERO_EVIDENCE: {
    label: "Required Knowledge decision に evidence がない",
    severity: "warning",
    area: "Knowledge",
    description: "knowledgePolicy=required の判断で selected support evidence がありません。",
  ...
  CONTEXT_DECISION_GH_UNAVAILABLE: {
    ...
    severity: "info",
    area: "Runtime",
    description: "GitHub CLI が未導入または未認証のため PR discard 状態を確認できません。",
    impact: "PR discard 由来の system feedback 自動回収だけが degraded になります。",
    action: "PR discard feedback を使う場合は gh を導入し認証してください。",
  },
  ...
  KNOWLEDGE_UNKNOWN_INTENT_TAGS: {
    label: "未知の intent tag が存在する",
    severity: "warning",
    area: "Knowledge",
  ...
  KNOWLEDGE_NEGATIVE_WITHOUT_ORIGIN: {
    ...
    area: "Knowledge",
    description:
      "polarity が negative なのに、関連する origin link、source evidence、legacy sourceDocumentUri のいずれも登録されていません。",
    impact:
      "ネガティブな指示（回避・確認手順）の背景や根拠（修正された課題、バグ、source evidence、legacy metadata など）が追跡できなくなります。",
    action:
      "該当する negative knowledge に origin link、source evidence、sourceDocumentUri のいずれかを紐付けるか、不要な項目を整理してください。",
  },
  ...
  AGENT_LOG_SYNC_LAUNCH_AGENT_NOT_INSTALLED: {
    label: "Resident daemon launch agent 未配置",
    severity: "warning",
    area: "Sync",
    description: "resident context-stilld LaunchAgent がインストールされていません。",
    impact: "定期同期が自動実行されません。",
    action: "`bun run automation:context-stilld -- install` を実行してください。",
  },
  AGENT_LOG_SYNC_LAUNCH_AGENT_NOT_LOADED: {
    label: "Resident daemon launch agent 未ロード",
    severity: "warning",
    area: "Sync",
    description: "resident context-stilld LaunchAgent がロードされていません。",
    impact: "定期同期が停止します。",
    action: "`bun run automation:context-stilld -- load` を実行してください。",
  },
  ...
  VIBE_DISTILLATION_LAUNCH_AGENT_NOT_INSTALLED: {
    ...
    severity: "warning",
    area: "Distillation",
    description: "resident context-stilld LaunchAgent が配置されていません。",
  ...
  VIBE_DISTILLATION_LAUNCH_AGENT_NOT_LOADED: {
    ...
    severity: "warning",
    area: "Distillation",
    description: "resident context-stilld LaunchAgent が未ロードです。",
  ...
  VIBE_DISTILLATION_FAILED_BACKLOG_HIGH: {
    label: "Vibe distillation 失敗backlog比率が高い",
    severity: "warning",
    area: "Distillation",
    description:
      "v2 queue tables 上で Vibe 由来の failed job が、全体比率または直近増分の閾値を超えています。",
    impact: "現在の distillation 品質低下、または再処理が必要な失敗増加を示します。",
    action:
      "Queue 管理画面または SQL で直近の failed job 理由を分類し、再実行対象を選別してください。",
  },
  VIBE_DISTILLATION_FAILED_BACKLOG_CRITICAL: {
    label: "Vibe distillation 失敗backlog比率が上限超過",
    severity: "warning",
    area: "Distillation",
    description:
      "v2 queue tables 上で Vibe 由来の failed job が、全体比率または直近増分の上限閾値を超えています。",
    impact: "現在の distillation 品質低下が大きく、再処理対象の判断が困難になりつつあります。",
  ...
  SOURCE_DISTILLATION_FAILED_BACKLOG_HIGH: {
    label: "Source distillation 失敗backlog比率が高い",
    severity: "warning",
    area: "Distillation",
    description:
      "v2 queue tables 上で Source 由来の failed job が、全体比率または直近増分の閾値を超えています。",
    impact: "現在の source distillation 品質低下、または再処理が必要な失敗増加を示します。",
    action:
      "Queue 管理画面または SQL で直近の failed job 理由を分類し、再実行対象を選別してください。",
  },
  SOURCE_DISTILLATION_FAILED_BACKLOG_CRITICAL: {
    label: "Source distillation 失敗backlog比率が上限超過",
    severity: "warning",
    area: "Distillation",
    description:
      "v2 queue tables 上で Source 由来の failed job が、全体比率または直近増分の上限閾値を超えています。",
    impact:
      "現在の source distillation 品質低下が大きく、再処理対象の判断が困難になりつつあります。",
    action: "失敗理由ごとに再実行、保留、破棄を分けて backlog を圧縮してください。",
  },
