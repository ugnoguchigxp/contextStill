...
function commonSystemLines(
  ...
  const lines = [
    ...
    `importance が ${groupedConfig.distillation.minCandidateImportance} 未満になる候補は compile-ready knowledge にせず、候補なしにする。`,
...
export function buildDistillationVerificationSystemPrompt(
  ...
  return [
    ...
    "confidence は根拠の強さ・検証確度、importance は次回以降の作業で再利用する価値として別々に評価する。",
    `importance が ${groupedConfig.distillation.minCandidateImportance} 未満になる候補は返さず、候補全体が低重要度なら candidates は空配列にする。`,
    "自然言語で返す場合は `TYPE: rule`、`TITLE: ...`、`BODY: ...` のラベル付きテキストにする。",
    "`TYPE / TITLE / BODY` のような見出し行だけを出さない。",
