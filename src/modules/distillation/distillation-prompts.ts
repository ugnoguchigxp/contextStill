...
function commonSystemLines(
  ...
  const lines = [
    ...
    "推奨出力は candidates 配列の最小 JSON。難しい場合は TYPE / TITLE / BODY のラベル付きテキストでもよい。",
    ...
    "body は短いラベルではなく、次回の coding agent が判断または実行できる具体的な制約・手順にする。十分に書けない場合は candidates を空配列にする。",
...
export function buildDistillationVerificationSystemPrompt(
  ...
  return [
    ...
    '推奨出力: {"candidates":[{"type":"rule|procedure","title":"...","body":"...","score":0.0,"confidence":0,"importance":0}]}',
    "JSON が難しい場合は TYPE / TITLE / BODY / SCORE(任意) のラベル付きテキストでもよい。",
    "省略できるキーは省略する。最終 knowledge に必要な情報は type / title / body のみ。",
