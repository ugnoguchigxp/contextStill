...
function commonSystemLines(
  ...
  const lines = [
    ...
    "出力は TYPE / TITLE / BODY のラベル付きテキストとする。",
    "candidates 配列以外の入れ子、refs 配列、rationale は原則出力しない。",
    "confidence と importance は判断可能な場合のみ 0 から 100 で付与する。",
    ...
    "body は最小 20文字の情報がないと取り扱わないので、次回の coding agent が判断または実行できる具体的な制約・手順をちゃんと作文しろ。十分に書けない場合は候補なしにする。",
...
export function buildDistillationExtractionSystemPrompt(
  ...
  return [
    ...
    "confidence / importance は最終値は後続の検証セッションで再評価する。",
    "",
    ...sourceSpecificSystemLines[sourceKind],
    ...(extraLines.length > 0 ? ["", ...extraLines] : []),
  ].join("\n");
...
export function buildDistillationVerificationSystemPrompt(
  ...
  return [
    ...
    "confidence と importance は判断できる場合だけ 0 から 100 で付ける。",
    "出力は TYPE / TITLE / BODY のラベル付きテキストとする。",
    "最終 knowledge に必要な情報は type / title / body のみ。",
