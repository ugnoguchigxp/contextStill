...
function commonSystemLines(
  ...
  if (options.includeExternalVerification) {
    lines.splice(
      ...
      "search_web は URL 発見用です。検索結果 snippet だけを根拠にせず、採用候補の一次ソース URL を fetch_content してから最終候補にする。",
      "search_web は最大 1 回、fetch_content は最大 3 回。失敗 URL があれば別の有望な URL を fetch_content し、search_web の言い換えを繰り返さない。",
      "search query は短く安定した公式名・API名・概念名を優先し、同じ意味の長い自然文 query を増やさない。",
...
export function buildDistillationVerificationSystemPrompt(
  ...
  return [
    ...
    "search_web は URL 発見用です。検索結果を受け取ったら、採用候補の一次ソース URL を 1 から 3 件選び、最終 candidates の前に fetch_content を呼ぶ。",
    "search_web は最大 1 回、fetch_content は最大 3 回。失敗 URL があれば別の有望な URL を試し、search_web の言い換え query を繰り返さない。",
    "search query は短く安定した公式名・API名・概念名を優先する。",
