...
function commonSystemLines(
  ...
  if (options.includeExternalVerification) {
    lines.splice(
      ...
      "fetch_content は複数回使ってよい。失敗 URL があれば別の有望な URL を fetch_content し、search_web の言い換えを繰り返さない。",
...
export function buildDistillationVerificationSystemPrompt(
  ...
  return [
    ...
    "fetch_content は複数回使ってよい。失敗 URL があれば別の有望な URL を試し、search_web の言い換え query を繰り返さない。",
