...
import type { LlmHealthResult, StartupPlan } from "./onboarding.types.js";
...
export async function checkPlanLlmHealth(plan: StartupPlan): Promise<LlmHealthResult> {
  ...
  try {
    ...
    if (plan.compile.openaiBaseUrl !== undefined)
      groupedConfig.openAi.apiBaseUrl = plan.compile.openaiBaseUrl;
    if (plan.compile.openaiModel !== undefined)
      groupedConfig.openAi.model = plan.compile.openaiModel;

    if (plan.compile.azureKey !== undefined)
      groupedConfig.azureOpenAi.apiKey = plan.compile.azureKey;
    if (plan.compile.azureBaseUrl !== undefined)
      groupedConfig.azureOpenAi.apiBaseUrl = plan.compile.azureBaseUrl;
    if (plan.compile.azureModel !== undefined)
      groupedConfig.azureOpenAi.model = plan.compile.azureModel;
    if (plan.compile.azureVersion !== undefined)
      groupedConfig.azureOpenAi.apiVersion = plan.compile.azureVersion;

    if (plan.compile.bedrockModel !== undefined)
      groupedConfig.bedrock.model = plan.compile.bedrockModel;
    if (plan.compile.bedrockRegion !== undefined)
      groupedConfig.bedrock.region = plan.compile.bedrockRegion;
    if (plan.compile.bedrockProfile !== undefined)
      groupedConfig.bedrock.profile = plan.compile.bedrockProfile;

    if (plan.compile.localLlmBaseUrl !== undefined)
      groupedConfig.localLlm.apiBaseUrl = plan.compile.localLlmBaseUrl;
    if (plan.compile.localLlmKey !== undefined)
      groupedConfig.localLlm.apiKey = plan.compile.localLlmKey;
    if (plan.compile.localLlmModel !== undefined)
      groupedConfig.localLlm.model = plan.compile.localLlmModel;
