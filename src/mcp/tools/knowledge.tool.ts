...
export const searchKnowledgeTool: ToolEntry = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      repoKey: { type: "string", minLength: 1, maxLength: 1024 },
...
export const updateKnowledgeTool: ToolEntry = {
  ...
  handler: async (args) => {
    ...
    const merged = {
      ...
      status: nextStatus,
      scope: (parsed.scope ?? existing.scope) as "repo" | "global",
      projectRef: parsed.projectRef ?? existing.projectRef ?? undefined,
      repoPath: parsed.repoPath ?? existing.repoPath ?? undefined,
      repoKey: parsed.repoKey ?? existing.repoKey ?? undefined,
