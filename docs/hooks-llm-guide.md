...
* **タイミング**: 作業を続けるか、修正して続けるか、reject / rollback / discard / escalate すべきかで停止しそうな時。特に、次の応答がユーザーへの確認質問になりそうで、自律的に続行できる余地がある時。
* **フックアクション**: shell hook ではなく、エージェントがユーザーへ質問する前に `context_decision` を pre-question gate として呼ぶ運用ルールとして扱います。
* **目的**: ブロッカー由来の判断を Knowledge evidence と現在の作業文脈から先に評価し、ユーザー確認が本当に必要な場合だけ escalation します。危険操作、PR作成前判断、未完了Todoや失敗後の継続可否などが対象です。`context_decision` が `reject` を返した場合は、対象アクションを継続せず、報告または確認待ちに切り替えます。
...
3. **ブロッカー判断や判断後フィードバックが漏れそうなとき**
   「ユーザー確認の前に `context_decision` を pre-question gate として呼び、ブロッカー由来の判断を記録してください。`reject` が返った場合は対象アクションを止め、その判断に従った作業が終わったら `context_decision_feedback` を保存してください。pre-commit 時点で結果が分かっていれば、そのタイミングでの記録が適しています」と案内する。
