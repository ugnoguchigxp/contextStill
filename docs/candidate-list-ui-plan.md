...
初期版では sort は `updatedAt desc, candidateIndex asc` に固定する。追加 sort は UI 利用が見えてから増やす。
...
| `cover` がなく `find_candidate_results` だけある | `candidate_only` |
| target がまだ pending / running 中 | `target_pending` |

`target_pending` と `candidate_only` が両方成立する場合は、target が running 系なら `target_pending` を優先する。停止・完了済み target に cover がない場合は `candidate_only` とする。
...
left join knowledge_items k
  on k.metadata ->> 'coverEvidenceResultId' = f.id
  or k.metadata ->> 'sourceUri' = concat('cover-evidence-result://', f.id)
```

Knowledge との join は JSONB metadata lookup になるため、実装時に migration を追加する。

```sql
create index knowledge_items_cover_evidence_result_id_idx
on knowledge_items ((metadata ->> 'coverEvidenceResultId'));

create index knowledge_items_metadata_source_uri_idx
...
2. `find_candidate_results` 起点の join query を作る。
3. `outcome` を repository 層で算出する。
4. `CandidateDiffSummary` を deterministic に算出する。
5. stats を同じ filter 条件から返す。
6. JSONB metadata lookup 用 index migration を追加する。
