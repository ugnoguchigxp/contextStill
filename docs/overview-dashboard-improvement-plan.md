...
この計画では Overview 専用 read model API を追加し、KPI と chart をすべて同じ DB snapshot から返す。UI の数値表示はその read model を正とし、runtime health だけは既存 `GET /api/doctor` を継続利用する。
...
- live refresh / WebSocket。初期版は TanStack Query の手動 refresh に限定する。
...
`doctor` は runtime health の正本として残す。`GET /api/overview` は `runDoctor()` を呼ばない。Overview 画面は metric/chart を `/api/overview` から、runtime health を `/api/doctor` から読む。画面側が list API の取得上限や graph snapshot の可視件数から KPI を合成しないことを境界にする。
...
| Compile run health by day | `ComposedChart` | ok/degraded/failed と avg duration を同時に追える |
| Vibe memory daily records | `LineChart` | ingestion spike と停滞が分かる |
| Source link coverage | `BarChart` | linked / unlinked knowledge を即座に見られる |
| Distillation queue | stacked `BarChart` | wiki / vibe の pending/running/completed を比較できる |

初期版では chart component を増やしすぎない。KPI cards + 6 charts + health panel までに絞る。
...
export const overviewDashboardSchema = z.object({
  ...
  charts: z.object({
    ...
    dynamicScoreBuckets: z.array(z.object({
      bucket: z.enum(["0", "0-1", "1-5", "5-10", "10+"]),
...
- 日別 series は直近 14 日を default にし、存在しない日は 0 で埋める。
- `knowledgeByStatusType` は存在しない status/type の組み合わせも 0 で返し、chart の系列が揺れないようにする。
- runtime health、doctor reasons、next actions は既存 `doctorReportSchema` を正とし、この schema には入れない。
...
Overview は中央寄せカード一覧ではなく、full-width dashboard にする。`AppShell` に Overview 専用の exact match 判定を追加し、`pathname === "/"` のときだけ `full-width` を付ける。`pathname.startsWith("/")` は全 route に一致するため使わない。
...
3. Chart grid
   ...
   - Distillation queue
4. Health panel
   - doctor reasons from `GET /api/doctor`
   - next actions from `GET /api/doctor`
...
| Wiki Pages | page count | indexed docs / fragments |
...
DB schema 変更と migration は不要。既存 table の aggregate read model として実装する。
...
3. `overview.page.tsx` を `useQuery({ queryKey: ["overview-dashboard"], queryFn: fetchOverviewDashboard })` に置き換える。
4. chart component を `web/src/modules/admin/components/overview-charts.tsx` に切り出す。
5. loading/error/empty state を追加する。

受け入れ条件:

- Overview の KPI と chart 表示値は `/api/overview` response だけから出る。
- `Vibe Memory 120` のような limit 由来の表示が消える。
- Dynamic Score が平均だけでなく bucket chart で読める。
- Doctor reasons は既存 `/api/doctor` query から見える。

### Phase 3: Layout / polish

1. Overview を full-width dashboard として扱う。
2. `app-shell.tsx` の full-width 判定に `pathname === "/"` を追加する。
3. `web/src/styles.css` に dashboard grid / chart card CSS を追加する。
4. desktop では chart 2 columns、mobile では 1 column にする。
5. 長い reason / next action がカード外にはみ出ないようにする。
6. Playwright で desktop / mobile screenshot を確認する。
...
- `test/api.routes.test.ts`
  ...
  - `/api/overview` が `runDoctor()` を呼ばないことを module boundary で維持する。
- `test/repositories.integration.test.ts`
  - seeded rows から knowledge breakdown、vibe total、source coverage、daily series を検証。
  - `MEMORY_ROUTER_RUN_DB_TESTS=1` 付きの safe integration DB だけで実行し、live DB には向けない。
...
- `bun run build:web`
- `bunx vitest run test/api.routes.test.ts`
- `bun run test:unit`

### Visual

`bun run build:web` 後、preview server で確認する。
...
- Overview の KPI は DB 実数だけを表示し、doctor の診断 window は health panel にだけ表示する。
...
改善: Overview API は doctor を置き換えない。doctor は health 判定の正本、Overview API は dashboard read model の正本とする。Overview API は `runDoctor()` を呼ばず、UI が既存 `/api/doctor` query の結果を health panel に表示する。
...
改善: 初期実装では DB timestamp を `date_trunc('day', created_at)` で集計し、API response に `timezone: "database"` を含めない代わりに tooltip で日付だけを表示する。JST 固定や user timezone 切り替えはこの計画の非目的とし、別計画で server config として扱う。
...
| 観点 | 判定 |
|---|---|
| 現行表示ズレの原因が特定されている | OK |
| 追加 API / repository / schema / UI / CSS / tests の対象ファイルが明記されている | OK |
| `doctor` と Overview read model の責務が分離されている | OK |
| chart library 導入範囲と bundle size rollback が書かれている | OK |
| DB migration の要否が明記されている | OK |
| full-width 判定の危険な `startsWith("/")` を避ける指示がある | OK |
| 実装後の検証コマンドと visual check が明記されている | OK |
