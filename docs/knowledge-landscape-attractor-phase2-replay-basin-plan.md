...
## 15. Phase 2C Replay Review UI 実装範囲

Phase 2C では、Phase 2B で追加した read-only replay comparison / sandbox action を
Graph UI の community view に接続する。目的は、Replay Corpus 上で見えている
Attractor / Negative Candidate / Dead Zone の説明力を、人間がレビューできる形にすることであり、
この段階では DB 更新や production ranking の変更は行わない。

### 15.1 UI で表示するもの

`graph.page.tsx` の community detail panel に `Replay Review` card を追加する。

- `GET /api/graph/landscape/replay/compare` を community view 表示時だけ取得する
- Graph UI では初期表示の重さを抑えるため `limit=25` を使う。API / CLI の広い分析既定値は `limit=100` のまま維持する
- compared run count / average overlap / used baseline lost / review item count を表示する
- `promotionGateSummary` / `scoreTuning` / `compileInterventionPlan` / `recompilePlan` を要約する
- `appliesToRefineCandidates` を Action Queue として最大 6 件表示する
- risky run を `lost_baseline` / `no_current_match` / `drifted` / negative feedback / used baseline lost で抽出し、最大 4 件表示する

### 15.2 明示的に行わないもの

- `appliesTo` の保存ボタンや自動修正ボタンは置かない
- promotion gate の強制適用は行わない
- score tuning を knowledge score に永続反映しない
- compile intervention を UI 操作から有効化しない
- replay compare 結果を別テーブルへ保存しない

### 15.3 受け入れ基準

1. Community view 以外では replay comparison API を呼ばない
2. Community view では `windowDays=30`, `limit=25`, `runStatus=all`, `currentLimit=12`, `includeRuns=true` で取得する
3. `Replay Review` card が Action Queue と Risky Runs を表示できる
4. Empty / loading state が崩れず、既存の Dynamic Health Card と Replay Health を壊さない
5. component test / graph route test / replay comparison service test / typecheck / format / build が通る

Phase 2C の完了時点では、Replay Corpus と sandbox action が UI 上でレビュー可能になる。
次の段階は、この UI から human review workflow へつなぐための review item 永続化または
manual action queue であり、自動修正ではなく review evidence の保存から始める。
