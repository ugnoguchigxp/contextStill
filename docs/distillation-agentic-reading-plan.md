...
context compression は永続テーブルに保存しない。読んだ segment の内容は LLM session 内の working context としてだけ使い、永続化するのは read event の locator、文字数、truncated flag、metadata に限定する。`distillation_context_notes` は追加しない。
...
`distillation_jobs` は、蒸留対象 subject の再開可能な作業単位である。source kind、subject ID、prompt version、phase、status、attempt count、last error、next retry time、metadata を持つ。
...
`distillation_read_events` は、LLM がどの locator を、どの目的で読んだかを保存する。source kind、subject ID、job ID、candidate ID、locator、read reason、char count、createdAt を持つ。本文の丸ごと保存は避け、必要なら短い excerpt に留める。

新規に `distillation_evidence_cache` を追加する。

`distillation_evidence_cache` は Web search / fetch の再利用用 cache である。query text、url、ok、fetchedAt、short excerpt、metadata を持つ。cache は検証補助であり、最終的な source link とは別に扱う。

既存の `distillation_candidates` には、必要なら metadata の中に job ID、read count、verification attempt count、retry policy を保存する。カラムを増やしすぎず、まずは既存 metadata を活用する。
...
`distillation_jobs` の推奨カラムは、id、source_kind、vibe_memory_id、source_fragment_id、prompt_version、status、phase、attempt_count、last_error、last_outcome_kind、next_retry_at、locked_by、locked_at、metadata、created_at、updated_at である。
...
読み取り回数や LLM call の上限は config と runtime context で管理する。job table に汎用の使用量 JSON は持たせない。

`distillation_read_events` のカラムは、id、job_id、candidate_id、source_kind、vibe_memory_id、source_fragment_id、locator、purpose、char_count、truncated、metadata、created_at である。

`distillation_evidence_cache` のカラムは、id、tool_name、query_text、url、ok、excerpt、metadata、fetched_at、created_at、updated_at である。
...
返す内容は、locator、heading または label、content excerpt、char count、truncated flag に限定する。返却文字数は runtime limit 内で切る。
...
この値は config で変更可能にする。ただし、最初から細かい phase 別設定を増やしすぎない。まずは共通 limit と、source/vibe の上書きだけにする。

上限を使い切った場合は、failed ではなく `limit_exhausted` として分類する。候補が弱いままなら rejected、まだ判断できないなら retryable paused にする。
...
`llm_timeout` は同じ入力で再試行しない。次回は read limit を小さくする、候補数を減らす、候補周辺 locator だけ読む、Web evidence を先に cache から使う、のいずれかに切り替える。
...
cache key は、tool name、正規化 query、URL を組み合わせる。

fetch content を丸ごと Knowledge に保存しない。cache に保存する本文は短い excerpt に留める。
