...
この計画は、Wiki の Markdown ファイルや vibe memory のような大きい入力を、LLM に一度で読ませる前提から脱却し、蒸留を小さく進められる処理に変えるための実装計画である。
...
この計画は、方針としては実装に進められる。ただし、初版のままでは実装者が判断する余地がやや大きい。特に、DB schema の最小形、Phase ごとの touched files、feature flag の初期値、既存 workflow との接続点、ユーザー判断が必要な運用値が不足していた。

本レビュー後の計画では、Phase 1 から実装に入れる。Phase 1 は再開可能 job と Doctor 可視化だけに限定するため、部分読みや圧縮の品質判断に入る前に、まず「動いているか、詰まっているか、再開できるか」を確認できる。

Phase 2 以降も実装可能だが、Agentic Reader を本番の既定経路にするタイミングだけは段階的に判断する。最初は feature flag で source/wiki の controlled run に限定し、Doctor と audit log で read event が安定して残ることを確認してから自動実行へ広げる。

## ユーザー判断が必要な事項

実装前に必ず決める必要があるものは少ない。未決でも conservative default で開始できる。

1 つ目は、Agentic Reader の初期有効範囲である。推奨は source/wiki の manual run のみ有効化し、自動 LaunchAgent では最初は無効にすることである。安定確認後に source/wiki の自動実行へ広げ、その後 vibe memory に広げる。

2 つ目は、HITL backpressure の挙動である。推奨は、draft backlog が既存 Doctor threshold を超えたら、新規 draft promotion を止め、verified candidate のまま残すことである。より積極的に進めたい場合は、importance が高い候補だけ promotion を許可する。

3 つ目は、context compression の保存範囲である。推奨は、圧縮メモを `distillation_context_notes` に保存するが、本文は短くし、source locator と hash を必ず持たせることである。圧縮メモを保存したくない場合は、job metadata のみに保存する軽量案も可能だが、再開性と Doctor 診断力は落ちる。

4 つ目は、Web evidence cache の TTL である。推奨は、初期値 7 日である。ただし、API 仕様、法律、価格、ライブラリ version など鮮度が重要な topic は短くする余地を残す。

5 つ目は、local-llm circuit breaker が開いたときに LaunchAgent job をどう扱うかである。推奨は、batch を paused にして次回 interval で再開することである。即時 retry は local-llm をさらに詰まらせる可能性があるため初期実装では避ける。
...
5. 圧縮済み working context と必要な元 locator から候補を作る。
6. 候補ごとに必要な部分だけ再読込する。
...
新規に `distillation_context_notes` を追加する。

`distillation_context_notes` は、LLM が読んだ内容を作業用に圧縮したメモである。job ID、candidate ID、note kind、body、source locators、createdAt を持つ。これは Knowledge の根拠ではなく、次の LLM call の入力を短くするための working memory である。
...
Phase 1 で追加する最小テーブルは `distillation_jobs` だけである。これにより、まず再開可能化と Doctor 可視化を入れる。
...
Phase 2 で `distillation_read_events` を追加する。推奨カラムは、id、job_id、candidate_id、source_kind、vibe_memory_id、source_fragment_id、locator、purpose、content_hash、char_count、truncated、metadata、created_at である。

Phase 3 で `distillation_context_notes` を追加する。推奨カラムは、id、job_id、candidate_id、note_kind、body、source_locators、metadata、created_at である。body は長文保存を避けるため、設定で最大文字数を制限する。

Phase 6 で `distillation_evidence_cache` を追加する。推奨カラムは、id、tool_name、query_hash、query_text、url、content_hash、ok、excerpt、metadata、fetched_at、created_at、updated_at である。

migration は各 Phase で分ける。複数 Phase のテーブルを 1 migration にまとめない。壊れた再ベース migration を避けるため、`bun run db:generate` の差分を必ず確認し、既存テーブルの `CREATE TABLE` が再生成されていないことを確認してから migrate する。
...
Phase 3 では、Agentic Reader session と context note を追加する。

対象は `src/modules/distillation/distillation-sessions.ts`、`src/modules/distillation/distillation-prompts.ts`、`src/modules/distillation/distillation-candidate-workflow.ts`、`src/modules/distillation/distillation-candidate.repository.ts`、関連 test である。
...
推奨する環境変数は、`MEMORY_ROUTER_DISTILLATION_AGENTIC_READER`、`MEMORY_ROUTER_SOURCE_DISTILLATION_AGENTIC_READER`、`MEMORY_ROUTER_VIBE_DISTILLATION_AGENTIC_READER` である。

初期値は false にする。manual controlled run では CLI option `--agentic-reader` で明示的に有効化できるようにする。

budget 設定は、`MEMORY_ROUTER_DISTILLATION_READ_LIMIT`、`MEMORY_ROUTER_DISTILLATION_READ_CHARS`、`MEMORY_ROUTER_DISTILLATION_LLM_CALL_LIMIT`、`MEMORY_ROUTER_DISTILLATION_WEB_SEARCH_LIMIT`、`MEMORY_ROUTER_DISTILLATION_FETCH_LIMIT` を想定する。

HITL backpressure は既存 Doctor の draft backlog threshold を初期値として使う。新しい設定を増やす場合は、`MEMORY_ROUTER_DISTILLATION_PROMOTION_BACKPRESSURE_DRAFTS` にする。
...
コンテキスト圧縮は、事前 summary ではなく、読んだ material を次の step に渡すための working memory として行う。
...
segment read は最大 5 回。

segment read の合計は 12000 文字まで。
...
importance が高く confidence も十分なものだけ promotion する設定を持つ。ただし score だけで機械的に候補を落とすのではなく、promotion の優先順位に使う。
...
読んだ内容だけを `distillation_context_notes` に圧縮する。

候補抽出 prompt は、working context と元 locator の組み合わせから候補を出すように変更する。

圧縮メモだけで promotion できないことを test する。

Phase 3 の完了条件は、`--agentic-reader --limit 1 --apply` で read event、context note、candidate が同じ job に紐づいて保存されることである。
