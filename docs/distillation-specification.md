...
再開可能化のため、`--apply` 実行時は対象ごとに `distillation_jobs` を作成または claim する。job は queued、running、paused、completed、skipped、failed の状態を持つ。local-llm や provider が不健康な場合は subject failed を増やし続けず、job を paused にして次回 interval で再開する。

source/wiki の manual run では Agentic Reader を opt-in で使える。`--agentic-reader` を付けた場合だけ、LLM は全文ではなく locator catalog を受け取り、必要な segment を `read_source_segment` で読む。自動 LaunchAgent と vibe memory では初期状態では無効である。
...
Agentic Reader の locator catalog、部分読み、read event 記録は `src/modules/distillation/distillation-reader.service.ts` と `src/modules/distillation/distillation-read-event.repository.ts` にある。

再開可能 job と circuit breaker / HITL backpressure は `src/modules/distillation/distillation-job.repository.ts` と `src/modules/distillation/distillation-job.service.ts` にある。

Web evidence cache は `src/modules/distillation/distillation-evidence-cache.repository.ts` にある。
...
source/wiki の蒸留は `bun run distill:sources` から実行する。内部では `src/cli/distill-sources.ts` が `distillSources` を呼ぶ。manual run で Agentic Reader を使う場合は `bun run distill:sources -- --agentic-reader` を使う。
...
source/wiki CLI は `--limit`、`--source-kind`、`--uri`、`--include-processed`、`--agentic-reader` を受け取る。現行で `--source-kind` として受け入れる値は wiki のみである。`--uri` はローカル path として解決される。
...
候補検証で使う tool loop の最大 round は既定 8 である。Web 検索や fetch の各 HTTP request timeout は既定 10000 ミリ秒である。tool result の最大文字数は既定 8000 文字である。検索結果件数は既定 3 件である。1 入力から最終的に扱う候補数の上限は既定 2 件である。

Web evidence cache TTL は `MEMORY_ROUTER_DISTILLATION_EVIDENCE_CACHE_TTL_SECONDS` で設定する。現在の既定は 604800 秒、つまり 7 日である。

Agentic Reader の読み込み上限は `MEMORY_ROUTER_DISTILLATION_READER_MAX_READS` と `MEMORY_ROUTER_DISTILLATION_READER_MAX_CHARS_PER_READ` で設定する。現在の既定は最大 8 read、1 read あたり 6000 文字である。

HITL backpressure の draft backlog 閾値は `MEMORY_ROUTER_DISTILLATION_PROMOTION_BACKLOG_THRESHOLD_COUNT` で設定する。現在の既定は 50 である。draft knowledge がこの数を超える場合、verified candidate の draft promotion は止まる。

候補の最小 importance は `MEMORY_ROUTER_DISTILLATION_MIN_CANDIDATE_IMPORTANCE` で設定する。現在の既定は 60 である。LLM が評価した importance がこの値未満なら、候補は低重要度として Knowledge 化しない。

Circuit breaker は `MEMORY_ROUTER_DISTILLATION_CIRCUIT_BREAKER_ENABLED`、`MEMORY_ROUTER_DISTILLATION_CIRCUIT_BREAKER_HEALTH_TIMEOUT_MS`、`MEMORY_ROUTER_DISTILLATION_CIRCUIT_BREAKER_PAUSE_SECONDS` で設定する。現在の既定は有効、health timeout 5 秒、pause 3600 秒である。

Agentic Reader の初期有効範囲は `MEMORY_ROUTER_SOURCE_DISTILLATION_AGENTIC_READER_MANUAL_ENABLED`、`MEMORY_ROUTER_SOURCE_DISTILLATION_AGENTIC_READER_AUTO_ENABLED`、`MEMORY_ROUTER_VIBE_DISTILLATION_AGENTIC_READER_MANUAL_ENABLED` で設定する。現在の既定は source/wiki manual true、source/wiki auto false、vibe memory manual false である。

失敗 run の再試行抑制時間は既定 21600 秒、つまり 6 時間である。直近 6 時間以内に failed run がある対象は、`--include-processed` なしでは通常の選択対象から外れる。

候補の採否は score threshold ではなく、検証結果、tool evidence、外部証拠、本文品質、LLM が評価した importance で決まる。importance は Knowledge 化する価値の判定にも使い、confidence は根拠の強さ・検証確度として保存する。
...
ただし、`promotion_paused_backpressure`、`batch_paused_circuit_breaker`、`job_already_running` は「処理済み skip」とは扱わない。これらは再開待ちまたは実行制御上の skip であり、次回 interval で再対象にできる。
...
通常の候補抽出 model call は `enableTools: false` で実行される。ここで tool call が返ってきても、runtime は通常の tool loop を使わない。

Agentic Reader が有効な source/wiki manual run では、候補抽出 model call だけ `read_source_segment` を使える。user message には本文全量ではなく locator catalog と source brief を渡す。LLM は必要な locator だけを読み、読んだ内容を session 内で統合して最終候補を返す。読んだ本文そのものや圧縮メモは永続化しない。永続化するのは read event の locator、hash、文字数、truncated flag、metadata である。

## 候補出力形式

LLM には厳密な JSON だけを求めない。推奨は最小 JSON だが、自然言語のラベル付き出力も受け入れる。

候補に必要な中心情報は type、title、body、confidence、importance である。confidence は根拠の強さ・検証確度、importance は次回以降の作業で再利用する価値として、LLM が別々に 0 から 100 で評価する。sourceRefs、evidenceRefs、rationale は parser 上は受け取れるが、prompt 上は原則出さない方針である。
...
JSON 風の候補が見つからない場合は、ラベル付き自然言語として読む。type、title、body、confidence、importance、rationale、sourceRefs、evidenceRefs のラベルを認識する。confidence は自信度または信頼度、importance は重要度という日本語ラベルでも受け取れる。body と rationale は複数行を受け取れる。

自然言語の見本は `TYPE: rule`、`TITLE: ...`、`BODY: ...`、`CONFIDENCE: ...`、`IMPORTANCE: ...` のようなラベル形式であり、`TYPE / TITLE / BODY` のような見出し行だけを候補として扱わない。見出し行や `rule` / `procedure` だけの行が title や body に混入した場合は、低品質候補として落とす。
...
confidence と importance は 0 から 100 に正規化される。空や不正値なら互換 fallback として confidence は 65、importance は 55 になるが、prompt は候補を返すたびに両方を評価するよう要求する。`80%` のような表記は数値部分を使う。0 より大きく 1 未満の小数は 100 倍される。1 は 1 として扱われる。
...
Agentic Reader を使った候補では、SOURCE_EVIDENCE に locator catalog だけを渡さない。候補抽出時に read_source_segment が返した tool result、または保存済み候補 metadata の readLocators から再構成した segment 本文を SOURCE_EVIDENCE に含める。これにより、検証セッションは candidate body だけでなく、実際に読んだ元 source 断片も確認できる。
...
最終 assistant content が空文字だった場合は、1 回だけ再 prompt する。再 prompt では、空配列または `TYPE: rule`、`TITLE: ...`、`BODY: ...`、`CONFIDENCE: ...`、`IMPORTANCE: ...` のラベル付きテキストを返すように促す。`TYPE / TITLE / BODY` のような見出し行だけは返さないように明示する。

最終 assistant content が null で tool call もない場合は、assistant content missing としてエラーになる。

## 蒸留 Tools

蒸留 runtime が持つ tool は、外部証拠 tool と部分読み tool に分かれる。

外部証拠 tool は search_web と fetch_content である。検証セッションはこの 2 つだけを使う。外部証拠 tool は audit log に残り、`--apply` 時の tool evidence gate の対象になる。

部分読み tool は read_source_segment と read_vibe_segment である。初期実装で実際に使うのは source/wiki manual Agentic Reader の read_source_segment である。部分読み tool は local source を読むための内部 tool であり、Web evidence としては扱わない。
...
search_web と fetch_content は Web evidence cache を使う。cache hit の場合も tool result として返り、metadata に cacheHit と cacheFetchedAt が入る。cache TTL は既定 7 日である。

外部証拠 tool call は成功・失敗にかかわらず audit log に記録される。search_web は DISTILLATION_WEB_SEARCH、fetch_content は DISTILLATION_FETCH_CONTENT として記録される。payload には callId、toolName、ok、durationMs、query または URL、resultCount、finalUrl、contentChars、redirectCount、cacheHit、error などが入る。
...
次に candidate の品質を確認する。title または body が tool 名だけの場合は rejectedLowQuality になる。body が短すぎる場合も rejectedLowQuality になる。title と body が同一の場合も rejectedLowQuality になる。confidence または importance が返っていない場合も、fallback 値で Knowledge 化せず rejectedLowQuality になる。

importance が `MEMORY_ROUTER_DISTILLATION_MIN_CANDIDATE_IMPORTANCE` 未満の場合も rejectedLowQuality になる。score は使わず、Knowledge 化する価値は importance で判断する。

title の最小長は、空白を除いて 3 文字である。body の最小長は、空白を除いて 24 文字である。

tool 名だけの判定対象は、現行 tool である search_web、fetch_content、read_source_segment、read_vibe_segment である。引用符や backtick、末尾の `()` が付いていても tool 名だけとみなす。
...
ただし、HITL backpressure が有効な状態では Knowledge 化に進まない。draft knowledge 件数が閾値を超えている場合、run は `promotion_paused_backpressure` として skipped になり、verified candidate は候補テーブルに残る。これは候補価値を捨てる skip ではなく、人間確認待ちの backlog を増やさないための promotion pause である。
...
skip 系の outcome は no_candidate、verification_no_candidate、missing_verification_tool_evidence、missing_external_evidence、invalid_candidate、mixed_candidate_rejections、candidate_rejected、promotion_paused_backpressure、batch_paused_circuit_breaker、job_already_running である。
...
invalid_candidate は、body が短すぎる、title/body が同一、tool 名だけ、confidence/importance 不足、importance が低すぎる、などの品質問題だけで rejected された場合である。

mixed_candidate_rejections は、複数種別の reject や failed candidate が混ざった場合である。

candidate_rejected は、上記に分類できない reject である。

promotion_paused_backpressure は、draft backlog が閾値を超えているため、verified candidate の draft promotion を止めた場合である。

batch_paused_circuit_breaker は、distillation LLM provider の health check が通らず、job を paused にした場合である。

job_already_running は、同じ subject/input の job が別 worker に claim されている、または next retry time 前で claim できない場合である。DB エラーや migration 不足はこの outcome にしない。
...
search_web と fetch_content は、それぞれ DISTILLATION_WEB_SEARCH と DISTILLATION_FETCH_CONTENT として記録される。検証 candidate の auditContext が渡るため、source kind、source ID、input hash、prompt version、candidate row ID、candidate index、candidate type、candidate title と関連づけられる。cache hit の場合も audit log は残り、payload で live fetch と区別できる。

read_source_segment と read_vibe_segment は Web evidence ではないため audit log には出さない。`--apply` 時に job ID がある場合だけ、`distillation_read_events` に locator、hash、文字数、truncated flag を保存する。
...
doctor は `distillation_jobs` の queued、running、paused、failed 件数、最新 pause 時刻、最新 error も表示する。UI では distillation panel に job stats が表示される。
...
run-once では project root に移動し、`bun run db:migrate` を実行してから該当蒸留 CLI を `--apply` 付きで実行する。初期実装では自動 source/wiki 蒸留に `--agentic-reader` は付けない。
...
job の paused は、next retry time を過ぎると次回 interval で claim 可能になる。Circuit breaker による pause は即時 retry しない。
...
蒸留は、local-llm 全体の queue や他クライアントの同時実行を制御しない。制御しているのは memory-router の distillation CLI 同士の同時実行、distillation job の claim、provider health に基づく pause である。
...
tool service は、search_web / fetch_content の cache hit を live request と同じ tool result 形式で返す。

reader service は、locator catalog から指定 segment だけを返し、read budget を超えた場合は失敗 result にする。

source/vibe distillation service は、job claim できない対象を job_already_running として skip し、job 作成エラーは failed として扱う。

source/vibe distillation service は、HITL backpressure 中に accepted candidate を draft knowledge に promotion しない。

doctor schema と UI は distillation job stats を扱える。
...
候補採否は score threshold ではなく、importance と validation gate が中心である。confidence/importance は LLM に厳密な JSON を強制せず、候補を返す場合は必ず評価させ、JSON でも自然言語ラベルでも正規化して使う。Knowledge 化する価値の足切りには importance を使い、score は使わない。
...
context compression 用の永続テーブルはない。Agentic Reader が読んだ内容は session 内 working context としてだけ扱い、DB には read event の locator/hash/文字数だけを残す。
