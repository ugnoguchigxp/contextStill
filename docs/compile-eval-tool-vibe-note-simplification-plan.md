...
- `drizzle/00xx_context_compile_evals.sql`
...
MCP registry に `compile_eval` を追加する。
...
`kind === "compile_eval"` の特別扱いは削除する。互換性を重視する場合でも、新規保存は禁止し、既存行は通常の legacy memo として表示するだけにする。
...
### Phase 5: 既存 compile_eval メモの扱い

選択肢は2つある。

推奨: one-time migration script を用意する。
...
- `metadata.migratedToContextCompileEvalId` を付けるか、移行記録を audit log に残す。
- Vibe Note では `compile_eval` legacy memo を通常メモとして扱うか、非表示にする。

代替: 過去データは移行しない。

今回の評価データがまだ少なく、互換コストを下げたい場合は、旧 `compile_eval` メモを legacy として残し、新規だけ `compile_eval` tool に切り替える。

## 5. initial_instructions / docs 更新
...
- `session_memo` rejects or ignores `score` in new input contract
