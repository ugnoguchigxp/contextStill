...
> Date: 2026-05-26
> Scope: sessionId に紐づく一時メモ MCP と Admin Web UI (`Vibe Note`)
...
- Web UI 表示名を `Vibe Note` にし、`Vibe Memory` の右隣に配置する方針を追加した。
- `sessionId` ベースで `Vibe Note` と `Vibe Memory` を相互リンクする方針を追加した。
...
内部名は `session_memo` のまま維持するが、Web UI の表示名は **Vibe Note** とする。Vibe Memory と同じ情報構造に寄せ、sessionId を軸に相互リンクできるようにする。

### Route / Navigation

- route: `/vibe-note`
- nav label: `Vibe Note`
- nav order: `Vibe Memory` の右隣。`app-shell.tsx` では `Vibe Memory`, `Vibe Note`, `Candidates` の順に並べる。
- files:
...
- `/vibe-note?sessionId=<encoded>`: 指定 session の note を初期選択する。
- `/vibe-memory?sessionId=<encoded>`: 指定 session の memory を初期選択する。既存 Vibe Memory page は query param を読み、該当 session があれば選択状態にする。
- `sessionId` は必ず `URLSearchParams` で encode/decode する。path param にはしない。
...
- header
  ...
  - last updated
  - `Open Vibe Memory` link (`/vibe-memory?sessionId=...`)
...
Vibe Note -> Vibe Memory:

- Vibe Note の session header に `Open Vibe Memory` link を表示する。
- link 先は `/vibe-memory?sessionId=<encoded>`。
- 対応する Vibe Memory session がない場合でも link は表示してよい。遷移先で empty state を出す。

Vibe Memory -> Vibe Note:

- Vibe Memory の session header に `Open Vibe Note` link を表示する。
- link 先は `/vibe-note?sessionId=<encoded>`。
- Vibe Note が存在しない session でも link は表示してよい。遷移先で empty slots / no notes を表示する。

Vibe Memory の tool usage からの deep link:

- Vibe Memory record の `metadata.toolCalls` に `session_memo` tool call が含まれる場合、`MemoryAuxiliaryPanel` に `Vibe Note` link を追加する。
- link 先は `/vibe-note?sessionId=<encoded>` を基本にする。
- tool call の arguments から `label` または `slot` が抽出できる場合は、`/vibe-note?sessionId=<encoded>&label=<encoded>` または `&slot=<number>` を付け、Vibe Note 側で該当 slot を highlight する。
- tool call の arguments は信頼せず、UI では link hint としてのみ使う。存在しない label / slot の場合は session top に遷移する。
- `session_memo` tool call を Vibe Memory 側で見つけるため、agent log sync が tool call name と arguments preview を metadata に残しているか確認する。残っていない場合は Phase 2 で tool-call extraction を補強する。
...
- 相互リンクは説明文ではなく header action として置く。画面内に長い使い方説明は追加しない。
- query param で渡された `label` / `slot` は、該当 card を一時 highlight するだけに留め、画面状態の永続保存には使わない。
...
1. `vibe-note.page.tsx` を追加する。
2. `App.tsx` に `/vibe-note` route を追加する。
3. `app-shell.tsx` に `Vibe Note` nav item を追加し、`Vibe Memory` の直後に置く。
4. `app-shell.tsx` の full-width 判定に `/vibe-note` を追加する。
5. `vibe-memory.page.tsx` に `sessionId` query param 初期選択と `Open Vibe Note` header link を追加する。
6. `vibe-memory.page.tsx` の `MemoryAuxiliaryPanel` に、`session_memo` tool call 由来の Vibe Note deep link を追加する。
7. `styles.css` に必要最小限の vibe note style を追加する。Vibe Memory の class を過度に流用して壊さないよう、共通化は必要になってから行う。
8. UI tests を `test/components/admin/vibe-note-page.test.tsx` と既存 Vibe Memory page test に追加する。
...
- `vibe-note-page.test.tsx`
  ...
  - `sessionId` query param selects matching session
  - `label` / `slot` query param highlights matching note
  - delete invalidates query
- `vibe-memory-page.test.tsx`
  - `sessionId` query param selects matching memory session
  - session header links to `/vibe-note?sessionId=...`
  - `session_memo` tool usage renders a Vibe Note deep link when label / slot is available
...
- `Vibe Note` appears immediately to the right of `Vibe Memory` in the main nav.
- Vibe Note and Vibe Memory can navigate to each other by `sessionId`.
- Vibe Memory records with `session_memo` tool usage can link to the corresponding Vibe Note session, and to a note label / slot when available.
...
bunx vitest run test/components/admin/vibe-note-page.test.tsx test/components/admin/vibe-memory-page.test.tsx
```
...
3. Open `/vibe-note?sessionId=<encoded>`.
4. Confirm the session appears in the sidebar.
5. Confirm slot cards show labels, body previews, metadata, and delete action.
6. Click `Open Vibe Memory` and confirm `/vibe-memory?sessionId=<encoded>` selects the matching session when it exists.
7. Open `/vibe-memory?sessionId=<encoded>` and confirm `Open Vibe Note` returns to the matching Vibe Note session.
8. If a Vibe Memory record includes `session_memo` tool usage, confirm the auxiliary panel links to Vibe Note.
9. Confirm `/vibe-memory` still renders unchanged without query params.
