...
- MCP の公開ツールは `spec/mcp-tools.md` 上で 7 個（`initial_instructions`, `context_compile`, `search_knowledge`, `register_candidate`, `search_memory`, `fetch_memory`, `doctor`）に固定されている。`initial_instructions` は引数なし、`context_compile` は `goal/changeTypes/technologies/domains` のみを受け取る。
...
- `spec/oss-onboarding-and-localization-plan.md` をこの実装計画として固定する。
...
3. `spec/mcp-tools.md` に言語切り替えのルールを追記する。
...
6. `spec/oss-onboarding-and-localization-plan.md` の進捗欄を更新し、完了 milestone を判別できるようにする。
...
```bash
git diff --check -- README.md README.jp.md spec/mcp-tools.md spec/oss-onboarding-and-localization-plan.md .env.example
bun run verify
```
...
- `spec/sqlite-provider-feasibility.md`
...
- `spec/ollama-provider-feasibility.md`
...
- `spec/subscription-provider-feasibility.md`
...
- README / README.jp / spec/mcp-tools.md が実装済み CLI と矛盾しない。
