...
- `crates/context-stilld/src/domains/queue_lifecycle/types.rs`
  - `QUEUE_SUPERVISOR` / `QUEUE_EXECUTOR_ONCE` が `bun run src/cli/queue-supervisor.ts` を指す。
- `crates/context-stilld/src/domains/queue_lifecycle/service.rs`
  - `start_executor_report` / `run_executor_once_report` が TS queue executor を起動できる。
- `crates/context-stilld/src/domains/resident_runtime/service.rs`
  - resident queue reconcile が条件次第で TS executor 起動へ到達する。
- `crates/context-stilld/src/domains/runtime_sidecars/service.rs`
  - `mcp-tool-dispatch-typescript-one-shot`
...
- `crates/context-stilld/src/domains/bootstrap/service.rs`
  - bootstrap report が `needs_typescript_doctor` / `bun run src/cli/startup.ts` を参照する。
- `crates/context-stilld/src/domains/doctor/service.rs`
  - doctor report が `delegatedFullDoctor: bun run src/cli/doctor.ts` を返す。
...
- Make `CONTEXT_STILL_RESIDENT_REQUIRE_RUST_ONLY=1` behavior the resident default.
- Remove or gate the resident path from `resident_runtime::reconcile_queue` to `queue_lifecycle::start_executor_report`.
...
- Remove `needs_typescript_doctor` from daemon readiness status or make it explicitly informational.
...
4. Remove stale MCP TS dispatch sidecar registry if no code path invokes it.
