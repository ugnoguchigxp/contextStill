...
The resident daemon owns queue scheduling/maintenance and the scheduled agent-log-sync trigger. Queue logs are written under app data logs, `queue inspect --json` reads live SQLite queue counts and provider leases from Rust, and `runtime sidecars --json` shows which TypeScript surfaces are UI-time work, manual one-shot work, or forbidden resident work. agent-log-sync parser/write and queue stale-state maintenance run in Rust. Finding and Episode use provider-pool leases; Finalize uses an independent local lane because it only needs the externally managed embedding service. ContextStill probes and calls that service but never starts or stops it. This prevents a Finding backlog from starving ready Finalize work. Transient embedding failures return Finalize jobs to `pending` with `next_run_at` backoff, while Knowledge, vector, source links, audit, and queue completion are committed atomically after embedding succeeds. TypeScript one-shot commands remain available as manual recovery tools.
...
| Embedding unavailable | Check `context-stilld doctor summary --json` fields `embedding.effectiveMode` and `embedding.daemon`; then check the externally managed endpoint and `CONTEXT_STILL_EMBEDDING_DIMENSION` |
