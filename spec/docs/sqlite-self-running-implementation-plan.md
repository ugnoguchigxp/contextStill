...
直近の実装で、SQLite backend は primary Knowledge/context flow の一部をすでに持っている。
...
ただし、これは「主要経路の先行移植」であり、「pgvector Docker 停止後も全体が self-running」とはまだ言えない。
...
- `doctor` の backend-aware 化。
- onboarding/setup/startup の SQLite-first 化。
- MCP smoke の SQLite mode 追加。
...
- `context-still-workflow.sqlite` を所有 store にする。
...
- `context-still-ingest.sqlite` を分離 store にする。
...
- runs/decision history は `context-still-runs.sqlite` に置く。
...
- PostgreSQL -> SQLite migration dry-run。
- PostgreSQL -> SQLite migration apply。
- SQLite core backup/restore。
