...
初期運用では入力契約を小さく保つ。`escalate` は 10% 未満に抑える目標を置き、低 confidence でも Evidence から前進可能な場合はユーザーへ戻さない。
...
呼び出し側は、判断点を抽象化しすぎない。MVP では `decisionPoint` に判断してほしい内容を直接書き、必要な場合だけ `retrievalHints` で Knowledge 検索の分類を補助する。
...
- 採用 Evidence が 0 件なら degraded とし、成功扱いしない。
...
採用根拠が 0 件の場合、成功扱いしない。

- evidence 0 件: degraded
