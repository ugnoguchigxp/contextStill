...
**context-still** は、AI コーディングエージェントのための local-first ナレッジエンジンです。コーディングセッション、Wiki、ドキュメントから再利用可能な **ルール** と **手順** を蒸留し、タスクごとに必要なコンテキストをコンパイルします。さらに、candidate、graph health、landscape risk、distillation queue をレビューする管理 UI / Control Plane を提供します。
...
<p align="center">
  <img src="artifacts/overview.webp" alt="context-still の管理 UI Overview 画面" width="980">
</p>

### 既存手法との比較

| 機能 | context-still | 一般的な RAG | CLAUDE.md / Cursor Rules |
|---|---|---|---|
...
context-still は、個人またはチームのコーディングエージェント運用を支える local-first プロジェクトです。ローカル MCP サーバー、REST API、管理 UI として利用できますが、ホスト型のマルチテナント SaaS ではありません。利用者自身の PostgreSQL/pgvector を起動し、蒸留された `draft` knowledge は `active` に昇格する前にレビューする前提です。
...
最も簡単な開始方法は、対話型の起動スクリプトを実行することです。これにより、環境の検証、必要な Docker コンテナの起動、マイグレーション、LLM の資格情報の確認、およびシステム全体のヘルスチェックが実行されます。

#### 対話型スタートアップスクリプトの特徴
- **デフォルトは dry-run**: ファイルの書き込みやデータベースの変更を行わず、安全に設定内容のシミュレーションとマスクされた `.env` の差分、実行計画を表示します。
- **安全な適用 (apply)**: `--apply` オプションを明示的に指定した場合のみ実行されます。書き換える前に自動的に `.env.bak-<timestamp>` バックアップファイルを作成します。
- **段階的な検証連鎖**: Docker接続検証、Drizzleマイグレーション、LLMヘルスチェック、スモークコンパイルテスト、最終的な `doctor` 健全性検証までを一貫して行います。

既存環境の保護を考慮し、デフォルトは **dry-run**（シミュレーション）モードで実行されます：
...
表示された実行計画とマスクされた `.env` の差分を確認後、実際に設定を適用します：

```bash
bun run startup -- --apply
```

手動で非対話的に設定を行いたい場合は、以下の従来の方法も引き続き利用可能です：
...
context-still は 3 段階のパイプラインで動作します。
...
context-still は [MCP（Model Context Protocol）](https://modelcontextprotocol.io/) サーバーを提供し、AI コーディングエージェントとシームレスに連携できます。
...
context-still は **evidence（証拠）** と **instruction（指示）** を明確に分離します。
...
デフォルトでは `context-still-db` Docker コンテナを使い、移行済みの既存ローカル環境では legacy の `memory-router-db` コンテナへフォールバックして、`backup/db_backup_<timestamp>.zip` を作成します。ローカル構成が `docker-compose.yml` と異なる場合は `BACKUP_DIR`、`CONTAINER_NAME`、`DB_USER`、`DB_NAME`、`DB_PASSWORD` を上書きできます。

---

## Embedding

context-still は 2 つの embedding プロバイダを自動フォールバック付きでサポートします。

| プロバイダ | 説明 | 設定 |
|---|---|---|
| **daemon**（デフォルト） | HTTP API embedding サービス | `CONTEXT_STILL_EMBEDDING_DAEMON_URL` |
| **cli** | Python CLI フォールバック（`e5embed.cli`） | `CONTEXT_STILL_LOCAL_LLM_EMBEDDING_*` |
...
- Wiki ページはローカルの `CONTEXT_STILL_SOURCE_CONTENT_ROOT` 配下に保存されます。
...
- できるだけ local-first に寄せる場合は `CONTEXT_STILL_DISTILLATION_PROVIDER=local-llm` を使い、検索 API key を設定しないでください。
...
| 変数名 | デフォルト | 説明 |
|---|---|---|
| `DATABASE_URL` | `postgres://...localhost:7889/context_still` | PostgreSQL 接続文字列 |
| `CONTEXT_STILL_SOURCE_CONTENT_ROOT` | `./wiki` | Wiki コンテンツディレクトリ |

### Embedding

| 変数名 | デフォルト | 説明 |
|---|---|---|
| `CONTEXT_STILL_EMBEDDING_PROVIDER` | `auto` | `auto`、`daemon`、`cli`、`disabled` |
| `CONTEXT_STILL_EMBEDDING_DAEMON_URL` | `http://127.0.0.1:44512` | Embedding daemon の URL |
| `CONTEXT_STILL_EMBEDDING_DIMENSION` | `384` | Embedding ベクトル次元数 |

### 蒸留（LLM）

| 変数名 | デフォルト | 説明 |
|---|---|---|
| `CONTEXT_STILL_LOCAL_LLM_API_BASE_URL` | `http://127.0.0.1:44448` | ローカル LLM API エンドポイント |
| `CONTEXT_STILL_DISTILLATION_PROVIDER` | `local-llm` | `local-llm`、`azure-openai`、`bedrock`、`auto` |
| `CONTEXT_STILL_DISTILLATION_FIND_CANDIDATE_PROVIDER` | `CONTEXT_STILL_DISTILLATION_PROVIDER` を継承 | `findCandidate` 専用 provider override。OpenAI/Azure で候補抽出する場合は `azure-openai`、必要に応じて `local-llm` / `bedrock` / `auto` |
| `CONTEXT_STILL_DISTILLATION_SEARCH_PROVIDERS` | `brave,exa` | `search_web` の provider 順序 |
| `CONTEXT_STILL_EXA_API_KEY` / `EXA_API_KEY` | 空 | Exa search API key |
...
| 変数名 | デフォルト | 説明 |
|---|---|---|
| `CONTEXT_STILL_CODEX_SESSION_DIR` | `~/.codex/sessions` | Codex セッションディレクトリ |
| `CONTEXT_STILL_CODEX_SESSION_DIRS` | 空 | 追加の Codex セッションディレクトリ（`,` / `;` 区切り） |
| `CONTEXT_STILL_CODEX_ARCHIVED_SESSION_DIRS` | 空 | 追加の Codex archived session ディレクトリ（`,` / `;` 区切り） |
| `CONTEXT_STILL_ANTIGRAVITY_LOG_DIR` | `~/.gemini/antigravity/brain` | Antigravity ログディレクトリ |
| `CONTEXT_STILL_ANTIGRAVITY_LOG_DIRS` | 空 | 追加の Antigravity ログディレクトリ（`,` / `;` 区切り） |
| `CONTEXT_STILL_AGENT_LOG_SYNC_INTERVAL_SECONDS` | `3600` | 同期間隔（秒） |
| `CONTEXT_STILL_AGENT_LOG_INITIAL_LOOKBACK_HOURS` | `168` | 初回取り込みの遡及時間 |
...
- 統合テストには `context_still_test` データベースが必要
