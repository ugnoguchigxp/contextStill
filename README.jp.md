...
- `initial_instructions`、`context_compile`、`compile_eval`、knowledge / memory search、candidate 登録の MCP tools
...
接続後は、project session 開始時に `initial_instructions` を一度だけ呼び、作業前に `context_compile`、作業後に `compile_eval` を使います。
