...
export type VibeGoal = {
  id: string;
  goalUri: string;
  goalAnchorRef: string;
  title: string | null;
  createdAt: string;
};

export type VibeMemoryMark = {
  id: string;
  goalId: string;
...
export async function fetchVibeGoals(): Promise<VibeGoal[]> {
  const json = await getJson<{ goals: VibeGoal[] }>("/api/vibe-memory/goals");
  return json.goals;
}

export async function fetchVibeMemoryContext(
  goalId: string,
  profile?: string[],
): Promise<VibeMemoryContextPack> {
  ...
  const json = await getJson<VibeMemoryContextPack[]>(`/api/vibe-memory/context?${params}`);
  return json[0];
}

export async function postMarkVibeMemory(input: {
  goalId: string;
  targetMemoryId: string;
  mark: string;
  note?: string;
  actorId: string;
}): Promise<VibeMemoryMark> {
  const json = await requestJson<{ mark: VibeMemoryMark }>("/api/vibe-memory/mark", "POST", input);
  return json.mark;
}

export async function postRecordVibeMemoryCapsule(input: {
  goalId: string;
  goalUri?: string;
  goalAnchorRef?: string;
  parentId?: string | null;
